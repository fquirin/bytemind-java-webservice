...
/**
 * Interface for account management services. Classes that implement the interface must check for user ID, access level and return error codes.
...
public interface AccountManager {
	 ...
	 * @return GUUID of new user
	 */
	public String createAdminAccount();
	
	/**
	 * Create a new user locally on the server via interactive console input (reduced security checks because, well if you have direct access to the server ...).
	 * @return GUUID of new user
	 */
	 ...
	 * "ticketid" (id to find token in database),<br>
	 * "type" (type of registration, e.g. via email),<br>
	 * "language" (language code submitted by client),<br>
	 * @return true/false
	 */
	 ...
	 * "ticketid" (id to find token in database),<br>
	 * "type" (type of registration, e.g. via email),<br>
	 * "new_pwd" (new password, hashed).<br>
	 * @return true/false
	 */
